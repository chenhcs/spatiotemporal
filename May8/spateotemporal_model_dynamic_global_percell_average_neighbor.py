...
class CustomAttentionSpatialCell(tf.keras.layers.Attention):
    ...
    def _calculate_values(self, vq, vk, vexp):
        scores1 = tf.einsum('blmn,blpn->blmp', vq, vk)
        v = tf.einsum('blmp,blp->blm', scores1, vexp)
        return v
...
class Decoder(tf.keras.layers.Layer):
  ...
  def call(self, target_exp, tf_exp, ligrecp_exp):
    ...
    ligand_exp = ligrecp_exp[:, :, :self.ligand_gene_size]
    receptor_exp_y_global = self.dec_spatial_layers_ligand(cell_embedding_ligrecp, cell_embedding_ligrecp, ligand_exp, (receptor_gene_embedding_global), (ligand_gene_embedding_global))
    receptor_exp_y_percell = self.dec_spatial_layers_ligand(cell_embedding_ligrecp, cell_embedding_ligrecp, ligand_exp, (receptor_gene_embedding_percell), (ligand_gene_embedding_percell))
    ...
    network_ligrecp_global = tf.einsum('blmn,bkpn->blkmp', receptor_gene_embedding_global, ligand_gene_embedding_global)
    ...
    network_ligrecp_percell = tf.einsum('blmn,bkpn->blkmp', receptor_gene_embedding_percell, ligand_gene_embedding_percell)
...
history = transformer.fit((ligrecp_exp, tf_exp, target_exp), {"output_1": target_exp_y, "output_2": target_exp_y, "output_3": target_exp_y, "output_4": target_exp_y, "output_5": recp_exp, "output_6": recp_exp, "output_7": target_exp_y, "output_8": target_exp_y, "output_9": target_exp_y, "output_10": target_exp_y, "output_11": recp_exp, "output_12": recp_exp},
                batch_size=3,
                epochs=eps)

predictions, predictions2, _, _, _, _, _, _, _, _, _, _ = transformer([ligrecp_exp[:1000], tf_exp[:1000], target_exp[:1000]]) #ligand, context, input, ligand_neighbor, graph
...
predictions, predictions2, _, _, _, _, _, _, _, _, _, _ = transformer([ligrecp_exp_val[:1000], tf_exp_val[:1000], target_exp_val[:1000]])
print(predictions.shape)
...
for b in range(1): #int(len(context)/1000) + 1
    predictions1, predictions2, _, _, _, _, _, _, _, _, _, _ = transformer([ligrecp_exp_val[b * 1000: (b+1) * 1000], tf_exp_val[b * 1000: (b+1) * 1000], target_exp_val[b * 1000: (b+1) * 1000]])
    ...
    for i in range(1):
        ...
        lig_vk_ = tf.reduce_mean(lig_vk_, axis=0)
...
head = 0
for b in range(int(len(ligrecp_exp_val)/1000) + 1):
    predictions1, predictions2, _, _, _, _, _, _, _, _, _, _ = transformer([ligrecp_exp[b * 1000: (b+1) * 1000], tf_exp[b * 1000: (b+1) * 1000], target_exp[b * 1000: (b+1) * 1000]])
