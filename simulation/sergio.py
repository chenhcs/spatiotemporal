    ...
    def build_graph (self, input_file_taregts, input_file_regs, shared_coop_state = 0):
            ...
            if (shared_coop_state <= 0):
                for row in reader:
                    ...
                    for regId, K, C_state in zip(row[2: 2 + nRegs], row[2+nRegs : 2+2*nRegs], row[2+2*nRegs : 2+3*nRegs]):
                        currInteraction.append((np.int64(regId), np.float64(K), np.float64(C_state), 0)) # last zero shows half-response, it is modified in another method
                        allRegs.append(np.int64(regId))
                        currParents.append(np.int64(regId))
                        self.graph_[np.int64(regId)]['targets'].append(np.int64(row[0]))

                    self.graph_[np.int64(row[0])]['params'] = currInteraction
                    self.graph_[np.int64(row[0])]['regs'] = currParents
                    self.graph_[np.int64(row[0])]['level'] = -1 # will be modified later
                    allTargets.append(np.int64(row[0]))

                    #if self.dyn_:
                    #    for b in range(self.nBins_):
                    #        binDict[b].append(gene(np.int64(row[0]),'T', b))
            else:
                for indRow, row in enumerate(reader):
                    nRegs = np.int64(np.float64(row[1]))
                    ##################### Raise Error ##########################
                    if nRegs == 0:
                        print ("Error: a master regulator (#Regs = 0) appeared in input")
                        sys.exit()
                    ...
                    for regId, K, in zip(row[2: 2 + nRegs], row[2+nRegs : 2+2*nRegs]):
                        currInteraction.append((np.int64(np.float64(regId)), np.float64(K), shared_coop_state, 0)) # last zero shows half-response, it is modified in another method
                        allRegs.append(np.int64(np.float64(regId)))
                        currParents.append(np.int64(np.float64(regId)))
                        self.graph_[np.int64(np.float64(regId))]['targets'].append(np.int64(np.float64(row[0])))

                    self.graph_[np.int64(np.float64(row[0]))]['params'] = currInteraction
                    self.graph_[np.int64(np.float64(row[0]))]['regs'] = currParents
                    self.graph_[np.int64(np.float64(row[0]))]['level'] = -1 # will be modified later
                    allTargets.append(np.int64(np.float64(row[0])))
        ...
        with open(input_file_regs,'r') as f:
            ...
            for row in reader:
                ...
                #if self.dyn_:
                #    for b in range(self.nBins_):
                #        binDict[b].append(gene(np.int64(row[0]),'MR', b))
