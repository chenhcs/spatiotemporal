...
class SpatialCellEmbedding(tf.keras.layers.Layer):
  ...
  def call(self, x):
    x = tf.einsum('blkm,mn->blkn', x, self.proj_kernel)
    return x
...
class LigandGeneEmbedding(tf.keras.layers.Layer):
  def __init__(self, d_model, gene_size):
    super().__init__()
    self.d_model = d_model
    self.gene_size = gene_size
  ...
  def call(self, x, gene_size):
    ...
    def frange(start, stop, step):
        while start <= stop:
            ...
            start += step
    '''
    x_token = tf.einsum('blkm,mn->blkmn', tf.ones_like(x, dtype=tf.float32), self.proj_kernel_token)
    x_value = tf.einsum('blkm,np->blkmn', x, self.proj_kernel_value)
    x_value = self.leaky_relu(x_value)
    x_value = tf.einsum('blkmn,np->blkmp', x_value, self.proj_kernel2_value) + self.scale_factor * x_value
    x_value = tf.nn.softmax(x_value, axis=-1)
    x_value = tf.einsum('blkmn,np->blkm', x_value, self.proj_kernel_disc_value)
    ...
    #x = x_token + x_value
    #x = tf.math.multiply(x_token, x_value)
    x = tf.einsum('blkmn,blkm->blkmn', x_token, x_value)
    bs = tf.shape(x)[0]
    ls = tf.shape(x)[1]
    ns = tf.shape(x)[2]
    x = tf.reshape(x, tf.concat([[bs * ls * ns], tf.shape(x)[3:]], axis=0))
    x = self.self_attention(x)
    #x = self.ffn(x)
    x = self.self_attention(x)
    #x = self.ffn(x)
    #x = self.self_attention(x)
    #x = self.ffn(x)
    x = tf.reshape(x, tf.concat([[bs, ls, ns], tf.shape(x)[1:]], axis=0))
...
class CustomAttentionSpatialCell(tf.keras.layers.Attention):
    def __init__(self, **kwargs):
        super().__init__()

    def _calculate_values(self, vq, vk, vexp):
        scores1 = tf.einsum('blmn,blkpn->blkmp', vq, vk)
        v = tf.einsum('blkmp,blkp->blkm', scores1, vexp)
        return v
    
    def _calculate_scores(self, query, key):
        scores = tf.einsum('bln,blkn->blk', query, key)
...
class BaseAttentionSpatialCell(tf.keras.layers.Layer):
  def __init__(self, **kwargs):
    super().__init__()
    self.mha = CustomAttentionSpatialCell(**kwargs)
    self.layernorm = tf.keras.layers.LayerNormalization()
    self.add = tf.keras.layers.Add()
...
class SpatialAttention(BaseAttentionSpatialCell):
  def call(self, x_query, context_key, context_vexp, x_vq, context_vk):
    attn_output, attn_scores = self.mha(
        inputs=[x_query, (context_vexp, x_vq, context_vk), context_key],
        use_causal_mask = True,
...
class DecoderSpatialLayer(tf.keras.layers.Layer):
  def __init__(self,
               *,
               d_model,
               num_heads,
               dff,
               dropout_rate=0.1):
    super(DecoderSpatialLayer, self).__init__()
  ...
  def call(self, x_query, context_key, context_vexp, x_vq, context_vk):
    x = self.spatial_attention(x_query, context_key, context_vexp, x_vq, context_vk)

    # Cache the last attention scores for plotting later
    self.last_attn_scores = self.spatial_attention.last_attn_scores
...
class Decoder(tf.keras.layers.Layer):
  def __init__(self, *, num_layers, d_model, target_gene_size, ligand_gene_size, receptor_gene_size, tf_gene_size, num_heads, dff,
    ...
    self.embed_cell_recp = CellEmbedding(d_model=d_model, gene_size=receptor_gene_size)
    self.embed_cell_ligand = SpatialCellEmbedding(d_model=d_model, gene_size=ligand_gene_size)

    #embed genes in each cell for (1) target genes, (2) TFs, (3) signaling gene (ligand + receptor) respectively
    self.embed_gene_target = GeneEmbedding(d_model=d_model, gene_size=receptor_gene_size + target_gene_size + tf_gene_size)
    self.embed_gene_tf = GeneEmbedding(d_model=d_model, gene_size=receptor_gene_size + tf_gene_size + target_gene_size)
    self.embed_gene_recp = GeneEmbedding(d_model=d_model, gene_size=receptor_gene_size + tf_gene_size + target_gene_size)
    self.embed_gene_ligand = LigandGeneEmbedding(d_model=d_model, gene_size=ligand_gene_size)
    ...
    self.dec_spatial_layers_ligand = DecoderSpatialLayer(d_model=receptor_gene_size, num_heads=num_heads,
                     dff=dff, dropout_rate=dropout_rate)

    self.last_attn_scores = None

  def call(self, target_exp, tf_exp, ligand_exp, recp_exp):
    cell_embedding_ligand = self.embed_cell_ligand(ligand_exp)
    ligand_gene_embedding_global, ligand_gene_embedding_percell = self.embed_gene_ligand(ligand_exp, self.ligand_gene_size)

    cell_embedding_recp = self.embed_cell_recp(recp_exp)
    recp_gene_embedding_global, recp_gene_embedding_percell = self.embed_gene_recp(tf.concat([recp_exp, tf_exp, target_exp], axis=-1), self.receptor_gene_size)

    cell_embedding_tf = self.embed_cell_tf(tf_exp)
    tf_gene_embedding_global, tf_gene_embedding_percell = self.embed_gene_tf(tf.concat([tf_exp, recp_exp, target_exp], axis=-1), self.tf_gene_size)

    cell_embedding_target = self.embed_cell_target(target_exp)  # Shape `(batch_size, seq_len, d_model)`.
    target_gene_embedding_global, target_gene_embedding_percell = self.embed_gene_target(tf.concat([target_exp, recp_exp, tf_exp], axis=-1), self.target_gene_size)

    receptor_exp = recp_exp
    ...
    target_exp_y2_global = self.dec_cross_layers_ligand(cell_embedding_target, cell_embedding_recp, receptor_exp, (target_gene_embedding_global), (recp_gene_embedding_global))
    ...
    target_exp_y2_percell = self.dec_cross_layers_ligand(cell_embedding_target, cell_embedding_recp, receptor_exp, (target_gene_embedding_percell), (recp_gene_embedding_percell))

    receptor_exp_y_global = self.dec_spatial_layers_ligand(cell_embedding_recp, cell_embedding_ligand, ligand_exp, (recp_gene_embedding_global), (ligand_gene_embedding_global))
    receptor_exp_y_percell = self.dec_spatial_layers_ligand(cell_embedding_recp, cell_embedding_ligand, ligand_exp, (recp_gene_embedding_percell), (ligand_gene_embedding_percell))
    ...
    self.recp_vk_global = recp_gene_embedding_global #signaling gene embedding (batch_size, seq_len, genes, d_model)
    self.lig_vk_global = ligand_gene_embedding_global
    ...
    self.recp_vk_percell = recp_gene_embedding_percell #signaling gene embedding (batch_size, seq_len, genes, d_model)
    self.lig_vk_percell = ligand_gene_embedding_percell

    network_tf_global = tf.einsum('blmn,bkpn->blkmp', target_gene_embedding_global, tf_gene_embedding_global)
    network_recp_global = tf.einsum('blmn,bkpn->blkmp', target_gene_embedding_global, recp_gene_embedding_global)
    network_ligrecp_global = tf.einsum('blmn,blkpn->blkmp', recp_gene_embedding_global, ligand_gene_embedding_global)
    network_tf_percell = tf.einsum('blmn,bkpn->blkmp', target_gene_embedding_percell, tf_gene_embedding_percell)
    network_recp_percell = tf.einsum('blmn,bkpn->blkmp', target_gene_embedding_percell, recp_gene_embedding_percell)
    network_ligrecp_percell = tf.einsum('blmn,blkpn->blkmp', recp_gene_embedding_percell, ligand_gene_embedding_percell)

    return target_exp_y1_global, target_exp_y2_global, target_exp_y1_percell, target_exp_y2_percell, receptor_exp_y_global, receptor_exp_y_percell, network_tf_global, network_recp_global, network_ligrecp_global, network_tf_percell, network_recp_percell, network_ligrecp_percell
...
class Transformer(tf.keras.Model):
  ...
  def call(self, inputs):
    ligand_exp, recp_exp, tf_exp, target_exp = inputs

    target_exp_y1_global, target_exp_y2_global, target_exp_y1_percell, target_exp_y2_percell, receptor_exp_y_global, receptor_exp_y_percell, network_tf_global, network_recp_global, network_ligrecp_global, network_tf_percell, network_recp_percell, network_ligrecp_percell = self.decoder(target_exp, tf_exp, ligand_exp, recp_exp)  # (batch_size, target_len, d_model)

    logits1_global = target_exp_y1_global
    logits2_global = target_exp_y2_global
    logist3_global = receptor_exp_y_global
    logits1_percell = target_exp_y1_percell
    logits2_percell = target_exp_y2_percell
    logist3_percell = receptor_exp_y_percell
...
print(tf_exp.shape, ligand_exp.shape, recp_exp.shape, target_exp.shape, target_exp_y.shape)
...
num_heads = 1
dropout_rate = 0.0
ligand_gene_size = ligand_exp.shape[-1]
receptor_gene_size = recp_exp.shape[-1] #ligrecp_exp.shape[-1] #
...
transformer.compile(
    loss={"output_1": masked_loss, "output_2": masked_loss, "output_3": masked_loss, "output_4": masked_loss, "output_5": masked_loss, "output_6": masked_loss, "output_7": l1_loss, "output_8": l1_loss, "output_9": l1_loss, "output_10": l1_loss, "output_11": l1_loss, "output_12": l1_loss},
    loss_weights={'output_1': 1.0, 'output_2': 1.0, 'output_3': 1.0, 'output_4': 1.0, 'output_5': 1.0, 'output_6': 1.0, 'output_7': 0.5, 'output_8': 0.5, 'output_9': 0.5, 'output_10': 0.5, 'output_11': 0.5, 'output_12': 0.5},
    optimizer=optimizer,
...
history = transformer.fit((ligand_exp, recp_exp, tf_exp, target_exp), {"output_1": target_exp_y, "output_2": target_exp_y, "output_3": target_exp_y, "output_4": target_exp_y, "output_5": recp_exp, "output_6": recp_exp, "output_7": target_exp_y, "output_8": target_exp_y, "output_9": target_exp_y, "output_10": target_exp_y, "output_11": recp_exp, "output_12": recp_exp},
                batch_size=3,
                epochs=eps)

predictions, predictions2, _, _, _, _, _, _, _, _, _, _ = transformer([ligand_exp[:1000], recp_exp[:1000], tf_exp[:1000], target_exp[:1000]]) #ligand, context, input, ligand_neighbor, graph
...
predictions, predictions2, _, _, _, _, _, _, _, _, _, _ = transformer([ligand_exp[:1000], recp_exp[:1000], tf_exp_val[:1000], target_exp_val[:1000]])
print(predictions.shape)
...
for b in range(1): #int(len(context)/1000) + 1
    predictions1, predictions2, _, _, _, _, _, _, _, _, _, _ = transformer([ligand_exp_val[b * 1000: (b+1) * 1000], recp_exp_val[b * 1000: (b+1) * 1000], tf_exp_val[b * 1000: (b+1) * 1000], target_exp_val[b * 1000: (b+1) * 1000]])
    ...
    ligand_vk1 = transformer.decoder.lig_vk_global
    ...
    weight_nt_ligrecp = np.zeros((ligand_vk1.shape[-2], recp_vk1.shape[-2]))
    for i in range(1):
        x_vq = vq[i]
        tf_vk = vk[i]
        recp_vk = lvk[i]
        ...
        weight_ligrecp = np.matmul(lig_vk_, np.transpose(recp_vk_))
        weight_nt_ligrecp += np.abs(weight_ligrecp)
    ...
    plt.clf()
    ax = plt.gca()
    ax.matshow(np.abs(weight_nt_ligrecp))
    plt.savefig('weight_test_triple/attention_weights_ligrecp_sample_' + str(b * 1000) + '.png')
    np.save('weight_test_triple/attention_weights_ligrecp_sample_' + str(b * 1000) + '.npy', weight_nt_ligrecp)
...
for b in range(int(len(recp_exp_val)/1000) + 1):
    predictions1, predictions2, _, _, _, _, _, _, _, _, _, _ = transformer([ligand_exp_val[b * 1000: (b+1) * 1000], recp_exp_val[b * 1000: (b+1) * 1000], tf_exp_val[b * 1000: (b+1) * 1000], target_exp_val[b * 1000: (b+1) * 1000]])
...
head = 0
for b in range(int(len(recp_exp_val)/1000) + 1):
    predictions1, predictions2, _, _, _, _, _, _, _, _, _, _ = transformer([ligand_exp_val[b * 1000: (b+1) * 1000], recp_exp_val[b * 1000: (b+1) * 1000], tf_exp[b * 1000: (b+1) * 1000], target_exp[b * 1000: (b+1) * 1000]])
