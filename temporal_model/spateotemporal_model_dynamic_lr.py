...
class Decoder(tf.keras.layers.Layer):
  ...
  def call(self, target_exp, tf_exp, ligrecp_exp):
    ...
    ligrecp_gene_embedding = self.embed_gene_ligrecp(tf.concat([ligrecp_exp, tf_exp, target_exp], axis=-1), self.ligand_gene_size + self.receptor_gene_size)
    ligand_gene_embedding = ligrecp_gene_embedding[:, :, :self.ligand_gene_size, :]
    receptor_gene_embedding = ligrecp_gene_embedding[:, :, self.ligand_gene_size:, :]

    cell_embedding_tf = self.embed_cell_tf(tf_exp)
    tf_gene_embedding = self.embed_gene_tf(tf.concat([tf_exp, ligrecp_exp, target_exp], axis=-1), self.tf_gene_size)

    cell_embedding_target = self.embed_cell_target(target_exp)  # Shape `(batch_size, seq_len, d_model)`.
    target_gene_embedding = self.embed_gene_target(tf.concat([target_exp, ligrecp_exp, tf_exp], axis=-1), self.target_gene_size)

    receptor_exp = ligrecp_exp[:, :, self.ligand_gene_size:]
    target_exp_y1 = self.dec_cross_layers_tf(cell_embedding_target, cell_embedding_tf, tf_exp, (target_gene_embedding), (tf_gene_embedding))
    target_exp_y2 = self.dec_cross_layers_ligand(cell_embedding_target, cell_embedding_ligrecp, receptor_exp, (target_gene_embedding), (receptor_gene_embedding))

    self.last_attn_scores = self.dec_cross_layers_tf.last_attn_scores
    self.x_vq1 = target_gene_embedding #target gene embedding (batch_size, seq_len, genes, d_model)
    self.tf_vk1 = tf_gene_embedding #tf gene embedding (batch_size, seq_len, genes, d_model)
...
class Transformer(tf.keras.Model):
  ...
  def call(self, inputs):
    ...
    target_exp_y1, target_exp_y2 = self.decoder(target_exp, tf_exp, ligrecp_exp)  # (batch_size, target_len, d_model)

    logits1 = target_exp_y1
    logits2 = target_exp_y2

    try:
      # Drop the keras mask, so it doesn't scale the losses/metrics.
      # b/250038731
      del logits1._keras_mask
...
tlength = 10 #length of time points in each sample
...
receptor_gene_size = recp_exp.shape[-1]
...
transformer.compile(
    loss={"output_1": masked_loss, "output_2": masked_loss},
...
predictions, predictions2 = transformer([ligrecp_exp_val[:1000], tf_exp_val[:1000], target_exp_val[:1000]])
print(predictions.shape)
...
for b in range(1): #int(len(context)/1000) + 1
    ...
    x_vq1 = transformer.decoder.x_vq1
    tf_vk1 = transformer.decoder.tf_vk1

    recp_vk1 = transformer.decoder.recp_vk
...
for b in range(int(len(ligrecp_exp_val)/1000) + 1):
    ...
    recp_vk = transformer.decoder.recp_vk
    print(x_vq.shape)
    print(tf_vk.shape)
    print(recp_vk.shape)
    #if True:
