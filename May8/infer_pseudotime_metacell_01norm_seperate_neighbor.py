...
def merge_neighbors_for_metacell(adata, k):
    d = pdist(adata_ori.obsm['spatial'])
    d2 = squareform(d)
    print(d2.shape)
    
    cluster_labels = adata.obs["clusters"]
    cluster_labels_pf = pd.DataFrame(adata.obs_names, index=adata.obs_names, columns=["ID"])
    cluster_labels_pf["cluster"] = cluster_labels.values
    cluster_to_cells = cluster_labels_pf.groupby("cluster")["ID"].apply(list).to_dict()
    print(cluster_to_cells)

    spatial_neighbors = {}
    for cluster in cluster_to_cells:
        id_in_cluster = np.array([int(x) for x in cluster_to_cells[cluster]])
        min_d = np.min(d2[id_in_cluster], axis=0)
        min_d[id_in_cluster] = float('inf')
        nnidx = np.argsort(min_d)
        spatial_neighbors[int(cluster)] = nnidx[1:k+1]
...
#find the nearest 10 neighbors of each meta cell
meta_spatial_neighbors = merge_neighbors_for_metacell(adata_ori, 10)
print(meta_spatial_neighbors)
...
for g in range(len(adata_ori.var)):
    exparray = adata_ori.X[:, g]
    std = np.std(exparray)
    if std > 0:
        adata_ori.X[:, g] = (adata_ori.X[:, g] - np.min(exparray)) / (np.max(exparray) - np.min(exparray)) #std
print(np.mean(adata_ori.X, axis=0))
print(np.min(adata_ori.X, axis=0))
...
for t in range(2):
    ...
    for p in all_paths:
        ...
        ligand = []
        for c in p:
            ligand.append(np.squeeze(adata_ori[meta_spatial_neighbors[c], all_ligand].X.toarray()))
